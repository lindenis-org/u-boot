...
int board_display_device_open(void)
{
	...
	__u32 output_type = 0;
	__u32 output_mode = 0;
	__u32 t_output_type = 0;
	__u32 t_output_mode = 0;
	__u32 auto_hpd = 0;
	__u32 err_count = 0;
	unsigned long arg[4] = {0};
	int node;
	...
	node = fdt_path_offset(working_fdt,"test_disp");

	err_count = 0;
	if (node >= 0) {
		/* getproc test_used, indicate whether work in test mode */
		if (fdt_getprop_u32(working_fdt, node, "test_used", (uint32_t*)&value) < 0) {
			pr_error("fetch script data test_disp.test_used fail, exit test mode\n");
			goto exit_test;
		} else
			pr_msg("test_disp.test_used=%d\n", value);
		...
		/* getproc output_disp, indicate which disp channel will be using */
		if (fdt_getprop_u32(working_fdt, node, "output_disp", (uint32_t*)&t_screen_id) < 0) {
			pr_error("fetch script data test_disp.output_disp fail\n");
			err_count ++;
		} else
			pr_msg("test_disp.output_disp=%d\n", t_screen_id);

		/* getproc output_type, indicate which kind of device will be using */
		if (fdt_getprop_u32(working_fdt, node, "output_type", (uint32_t*)&value) < 0) {
			pr_error("fetch script data test_disp.output_type fail\n");
			err_count ++;
		} else
			pr_msg("test_disp.output_type=%d\n", value);

		if(value == 0)
		{
			t_output_type = DISP_OUTPUT_TYPE_NONE;
		}
		else if(value == 1)
		{
			t_output_type = DISP_OUTPUT_TYPE_LCD;
		}
		else if(value == 2)
		{
			t_output_type = DISP_OUTPUT_TYPE_TV;
		}
		else if(value == 3)
		{
			t_output_type = DISP_OUTPUT_TYPE_HDMI;
		}
		else if(value == 4)
		{
			t_output_type = DISP_OUTPUT_TYPE_VGA;
		}
		else
		{
			pr_error("invalid output_type %d\n", value);
		...
		/* getproc output_mode, indicate which kind of mode will be output */
		if (fdt_getprop_u32(working_fdt, node, "output_mode", (uint32_t*)&t_output_mode) < 0) {
			pr_error("fetch script data test_disp.output_mode fail\n");
			err_count ++;
		} else
			printf("test_disp.output_mode=%d\n", output_mode);
		...
		if(err_count == 0) {
			...
			arg[2] = t_output_mode;
			disp_ioctl(NULL, DISP_DEVICE_SWITCH, (void *)arg);
			pr_notice("disp%d device type(%d) enable , in TestMode\n", t_screen_id, t_output_type);
		}
	}
...
exit_test:
	if (work_mode == 0) {
		...
		arg[2] = output_mode;
		disp_ioctl(NULL, DISP_DEVICE_SWITCH, (void *)arg);
		pr_notice("disp%d device type(%d) enable , in NormalMode\n", screen_id, output_type);
	}
